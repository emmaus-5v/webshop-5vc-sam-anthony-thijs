...
function getProducts(request, response) {
  ...
  if (category_id > 0) {
    ...
    const sqlOpdracht = db.prepare('SELECT COUNT(accesoireId_tabel.dieren_soort) AS "hoeveelheid_a", products.id, products.name, diersoorten.ds_naam, products.kleur, products.ras, products.gewicht, products.code, products.description, products.price, geslachten.g_naam, accesoires.a_naam, accesoires.a_kleur, accesoiresoorten.acs_naam, voedselsoorten.vs_naam, voedsel.merk, voedsel.hoeveelheid FROM products LEFT JOIN diersoorten ON products.diersoort = diersoorten.ds_id LEFT JOIN accesoireId_tabel ON products.diersoort = accesoireId_tabel.dieren_soort LEFT JOIN accesoires ON accesoireId_tabel.accesoires_id = accesoires.a_id LEFT JOIN accesoiresoorten ON accesoires.soort = accesoiresoorten.acs_id LEFT JOIN geslachten ON products.geslacht = geslachten.g_id LEFT JOIN voedselId_tabel ON products.diersoort  = voedselId_tabel.dieren_soort LEFT JOIN voedsel ON voedselId_tabel.voedsel_id = voedsel.v_id LEFT JOIN voedselsoorten ON voedsel.voedselsoort = voedselsoorten.vs_id GROUP BY products.id ORDER BY products.name')
...
function getProductById(request, response) {
  ...
  const sqlOpdracht = db.prepare('SELECT COUNT(accesoires.a_id) AS "hoeveelheid_a", products.id, products.name, diersoorten.ds_naam, products.kleur, products.ras, products.gewicht, products.code, products.description, products.price, geslachten.g_naam, accesoires.a_naam, accesoires.a_kleur, accesoiresoorten.acs_naam, voedselsoorten.vs_naam, voedsel.merk, voedsel.hoeveelheid FROM products LEFT JOIN diersoorten ON products.diersoort = diersoorten.ds_id LEFT JOIN accesoireId_tabel ON products.diersoort = accesoireId_tabel.dieren_soort LEFT JOIN accesoires ON accesoireId_tabel.accesoires_id = accesoires.a_id LEFT JOIN accesoiresoorten ON accesoires.soort = accesoiresoorten.acs_id LEFT JOIN geslachten ON products.geslacht = geslachten.g_id LEFT JOIN voedselId_tabel ON products.diersoort  = voedselId_tabel.dieren_soort LEFT JOIN voedsel ON voedselId_tabel.voedsel_id = voedsel.v_id LEFT JOIN voedselsoorten ON voedsel.voedselsoort = voedselsoorten.vs_id WHERE id = ? GROUP BY products.id ORDER BY products.name')
