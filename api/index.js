...
function getProducts(request, response) {
  ...
  if (category_id > 0) {
    ...
    const sqlOpdracht = db.prepare('SELECT products.name, products.diersoort, products.kleur, products.ras, products.gewicht, products.code, products.description, products.price, geslachten.naam, accesoires.naam, accesoires.kleur, accesoiresoorten.naam, voedselsoorten.naam, voedsel.merk, voedsel.hoeveelheid FROM products JOIN accesoireId_tabel ON products.id = accesoireId_tabel.dieren_soort JOIN accesoires ON accesoireId_tabel.accesoires_id = accesoires.a_id JOIN accesoiresoorten ON accesoires.soort = accesoiresoorten.acs_id JOIN geslachten ON products.geslacht = geslachten.g_id JOIN voedselId_tabel ON products.id  = voedselId_tabel.dieren_soort JOIN voedsel ON voedselId_tabel.voedsel_id = voedsel.v_id JOIN voedselsoorten ON voedsel.voedselsoort = voedselsoorten.vs_id')
