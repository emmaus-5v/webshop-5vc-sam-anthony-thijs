...
function getProducts(request, response) {
  ...
  if (category_id > 0) {
    ...
    const sqlOpdracht = db.prepare('SELECT products.id, products.name, products.diersoort, products.kleur, products.ras, products.gewicht, products.code, products.description, products.price, geslachten.g_naam, accesoires.a_naam, accesoires.a_kleur, accesoiresoorten.acs_naam, voedselsoorten.vs_naam, voedsel.merk, voedsel.hoeveelheid FROM products LEFT JOIN accesoireId_tabel ON products.id = accesoireId_tabel.dieren_soort LEFT JOIN accesoires ON accesoireId_tabel.accesoires_id = accesoires.a_id LEFT JOIN accesoiresoorten ON accesoires.soort = accesoiresoorten.acs_id LEFT JOIN geslachten ON products.geslacht = geslachten.g_id LEFT JOIN voedselId_tabel ON products.id  = voedselId_tabel.dieren_soort LEFT JOIN voedsel ON voedselId_tabel.voedsel_id = voedsel.v_id LEFT JOIN voedselsoorten ON voedsel.voedselsoort = voedselsoorten.vs_id GROUP BY products.diersoort ORDER BY products.name')
